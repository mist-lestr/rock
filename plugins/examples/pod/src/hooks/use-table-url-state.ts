...
export function useTableUrlState(
  ...
  const initialColumnFilters: ColumnFiltersState = useMemo(() => {
    ...
    for (const cfg of columnFiltersCfg) {
      const raw = ({} as SearchRecord)[cfg.searchKey]
  ...
  const pagination: PaginationState = useMemo(() => {
    const rawPage = ({} as SearchRecord)[pageKey]
    const rawPageSize = ({} as SearchRecord)[pageSizeKey]
  ...
  const onPaginationChange: OnChangeFn<PaginationState> = (_updater) => {
  ...
  const [globalFilter, setGlobalFilter] = useState<string | undefined>(() => {
    ...
    const raw = ({} as SearchRecord)[globalFilterKey]
  ...
  const ensurePageInRange = (
    ...
    const currentPage = ({} as SearchRecord)[pageKey]
