...
import { cva } from 'class-variance-authority'; // still usable, no React
...
export class UIBadge extends LitElement {
  ...
  connectedCallback() {
    ...
    if (this.shadowRoot) adoptStyles(this.shadowRoot, [stylesheet, UIBadge.styles]);
  }

  render() {
    const classes = badgeVariants({ variant: this.variant as any });
